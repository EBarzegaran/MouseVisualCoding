...
def extract_probeinfo(session,probe_id,Resultspath):
    ...
    A.rename(columns={'anterior_posterior_ccf_coordinate': 'AP_CCF', 'left_right_ccf_coordinate': 'ML_CCF', 'dorsal_ventral_ccf_coordinate': 'DV_CCF'},inplace=True)
    
    # create folder if not exist
    if not os.path.exists(os.path.join(Resultspath,'MatlabData')):
        oos.path.join(Resultspath,'MatlabData')
        
    # Save the result
    sio.savemat(os.path.join(Resultspath,'MatlabData','{}_ProbeInfo.mat'.format(probe_id)), 
...
def prepare_condition(session,lfp,probe_id,cond_name,Resultspath,Prestim,down_rate):
    ...
    # select condition 2088 -> contrast .8 and orientation 0
    Y = np.squeeze(lfp_cond[:,:,:,np.array(results['conditioninfo'])==2088])
    Y2 = np.squeeze(lfp_cond[:,:,:,np.array(results['conditioninfo'])==2086])
    Y = np.concatenate((Y,Y2),axis=2)

    # select VISp electrodes
    Y = Y[intervals[intervals.shape[0]-3]:intervals[intervals.shape[0]-2]+1];
    # prepare data for STOK
    Y = np.moveaxis(Y,-1,0) 
    Y = Y[:,np.arange(0,Y.shape[1],1),:];

    # downsampling the signal
    #Here I will downsample the signal to allow better estimation of FC in lower frequencies using AR models and also reduce the size:
    Y = signal.decimate(Y,down_rate)
    Times = Times[np.arange(0,Times.shape[0],down_rate)]
    dSRate = SRate/down_rate;
    YP = Y.copy();
    for i in range(0,np.asarray(YP.shape)[1]):
        YP[:,i,:] += (i*Y.max()*.3)
    fig, axs = plt.subplots(figsize=(12,8))
    #axs = plt.subplots(figsize=(12,8))
    P = axs.plot(np.nanmean(YP,axis=0).transpose());

    num_time_labels = 12
    time_label_indices = np.around(np.linspace(1, len(Times), num_time_labels)).astype(int) - 1
    time_labels = np.round(Times[time_label_indices],2);
    axs.set_xticks(time_label_indices);
    axs.set_xticklabels(time_labels);
    axs.set_xlim([0,np.where(np.round(Times,decimals=3)==1.9)[0][0]])
    #plt.legend(P,np.arange(0,12))
    plt.title(structure_acronyms[structure_acronyms.shape[0]-2])
    fig.savefig('{}/Probe_{}_{}_bipolar_electrode_sel.png'.format(Resultspath,probe_id,cond_name),dpi=300)
    ...
    cnd_info = list(map(lambda x: presentations[presentations['stimulus_condition_id']==x].iloc[0],cnd_id))# each condition info
...
def RF_mapping_plot(session,lfp,probe_id,Resultspath):
